...
parser.add_argument('--scratch', default='C:/Users/user/Downloads/rethinking-network-pruning/l1-norm-pruning/Pruned_Models/modelaki.pth.tar', type=str, help='the PATH to the pruned model')
...
def save_checkpoint(state, is_best, filepath):
    torch.save(state, os.path.join(filepath, 'typeD_resnet56.pth.tar'))
    if is_best:
        shutil.copyfile(os.path.join(filepath, 'typeD_resnet56.pth.tar'), os.path.join(filepath, 'best_typeD_resnet56.pth.tar'))
...
if __name__ == '__main__':
    ...
    if(args.scratch):
        ...
        model = resnet_cifar10.resnet(cfg=checkpoint['cfg'], depth=56)
        print("Completely loaded the pruned resnet 56")
    else:
        model = resnet_cifar10.resnet(groups=args.NumOfGroups)
        print("EDW MPAINEI")
    model.cuda()
