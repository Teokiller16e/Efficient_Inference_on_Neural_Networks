...
import resnet
from resnet_l1_norm_separable import resnet34
...
parser.add_argument('--resume', default='C:/Users/user/Downloads/rethinking-network-pruning/l1-norm-pruning/From_Scratch/groups4_sepa.pth.tar', type=str, metavar='PATH', help='path to latest checkpoint (default: none)')
parser.add_argument('-e', '--evaluate', dest='evaluate', action='store_true',help='evaluate model on validation set')
parser.add_argument('--pretrained', dest='pretrained', action='store_true',help='use pre-trained model')
parser.add_argument('--world-size', default=1, type=int,help='number of distributed processes')
parser.add_argument('--dist-url', default='tcp://224.66.41.62:23456', type=str,help='url used to set up distributed training')
parser.add_argument('--dist-backend', default='gloo', type=str,help='distributed backend')
parser.add_argument('--save', default='C:/Users/user/Downloads/rethinking-network-pruning/l1-norm-pruning/From_Scratch', type=str, metavar='PATH', help='path to save prune model (default: current directory)')
parser.add_argument('--scratch', default='C:/Users/user/Downloads/rethinking-network-pruning/l1-norm-pruning/Pruned_Models/resnet34_TYPE_D.pth.tar', type=str, metavar='PATH', help='the PATH to the pruned model')
...
def main():
    ...
    if args.scratch:
        ...
        test_model = resnet.resnet34(cfg=checkpoint['cfg'])
        count_parameters(test_model)
        model = resnet34(cfg=checkpoint['cfg'], groups=args.NumOfGroups)
        print("Completely loaded the pruned resnet 34")
    else:
        model=resnet34(groups=args.NumOfGroups)

    optimizer = torch.optim.SGD(model.parameters(), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)


    #flop_ar = count_model_param_flops(model)

    # Define model and set optimizers/criterion/loss etc.

    print("PASSED CHECKPOINT CONFIGURATION")
    par_ar = count_parameters(model)
    print(par_ar)
...
def save_checkpoint(state, is_best):
    torch.save(state, os.path.join(args.save, 'groups4_sepa.pth.tar'))
    if is_best:
        shutil.copyfile(os.path.join(args.save, 'groups4_sepa.pth.tar'), os.path.join(args.save, 'best_groups4_sepa.pth.tar'))
